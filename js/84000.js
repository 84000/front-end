...
jQuery(document).ready(function($) {
	...
	setTimeout(function(){
		...
		// -----------------------------------------
		(function ($) { 
			$.scrollToAnchor = function (hash, delay, parent, offset, callback, link) {
				...
				if(!legacyLink() && $("html.screen").length) {
					...
					if(hash){
						...
						if($target.length && !$target.closest(".partial").length){
							...
							// New for Reading Room 2.1.0
							// Redirect to part if hash not in DOM
							// --------------------------------------
							// console.log(hash + ' not present in DOM');
							// console.log(window.location.search);
							// console.log(link.search);

							// The hash is not in this DOM
							// So use the link if it has a part parameter
							// ------------------------------------------
							var urlPart = $.getUrlParameter('part');
							var linkPart = link ? $.getUrlParameter('part', link) : '';
							//console.log(urlPart);
							//console.log(linkPart);

							if(urlPart == 'all' || (urlPart && urlPart == linkPart)){
								// The page is complete - or already on the requested part
								// do nothing
								callback();
							}
							else if(link){
								...
								if(linkPart) {
									...
									// Maintain the mark
									var linkMarkParam = $.getUrlParameter('mark', link);
									var markParam = linkMarkParam ? "&mark=" + linkMarkParam : '' ;
									//console.log(partParam + markParam + hash);
									$.wait("Loading section...");
									location.href = partParam + markParam + hash;
								}
							}
							else if(!urlPart){
								// Add a part to the url
								var partParam = "?part=" + hash.substr(hash.indexOf('#') + 1);
								//console.log(partParam + hash);
								$.wait("Loading section...");
								location.href = partParam + hash;
							}
							else if(callback) {
								callback();
							}
		...
		$(document).on("click",'a.scroll-to-anchor', function(e) {
			...
			if(window.location.pathname.replace(/^\//,'') == this.pathname.replace(/^\//,'') && window.location.hostname == this.hostname) {
				...
				$.scrollToAnchor(this.hash, 0, null, null, postScrollToAnchor, this);
	    	...
	    	(function ($) { 
		    	...
		    	$.fn.bookmarkData = function () {
		    	    ...
		    	    return {
		    	    	...
		    	    	'hash': this.attr('href') ? this.attr('href') : "#top", 
